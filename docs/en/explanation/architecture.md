...
**MCP Server** (`src/agentcloak/mcp/`): Built with [FastMCP](https://github.com/modelcontextprotocol/python-sdk). Runs as a stdio MCP server, exposing 38 tools that map to daemon HTTP endpoints. The MCP server auto-starts the daemon on the first request, sharing the same `DaemonClient` (async mode) as the CLI uses in sync mode. Tool responses are rendered through `core/text_renderers` — the same renderer the CLI uses — so MCP returns agent-readable text instead of JSON strings. `agentcloak_screenshot` returns an MCP `ImageContent` so multimodal LLMs read the pixels directly.
