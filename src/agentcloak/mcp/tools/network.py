...
def register(mcp: FastMCP, client: DaemonClient) -> None:
    ...
    async def agentcloak_network(
        since: str = "0", pending: bool = False, filter: str = ""
    ) -> str:
        ...
        Args:
            pending: Only requests still in flight, including streams.
            filter: Case-sensitive URL glob.
        ...
        return await format_call(
            client.network(since=since, pending=pending, filter=filter),
            render_network_text,
        )
